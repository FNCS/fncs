...
! enough to hold names, event names, event values, etc.
  ...
  character ( len=1 ) :: event_name
  character ( len=1 ) :: event_value
...
!  write (*,'(A)') trim(config)
  ...
  if ( fncs_is_initialized ( ) .NE. 1 ) then
  ...
  do time_requested = 2, 10, 20
    time = fncs_time_request ( time_requested )
  ...
  call fncs_finalize ( )
  if ( fncs_is_initialized ( ) .NE. 0 ) then
